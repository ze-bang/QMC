...
// Base QuantumMonteCarlo implementation
QuantumMonteCarlo::QuantumMonteCarlo(std::shared_ptr<SpinModel> model, const Parameters& params)
...
void QuantumMonteCarlo::registerObservable(const std::string& name, ObservableFunction func) {
    observableFunctions_[name] = func;
    observableValues_[name] = 0.0;
    observableErrors_[name] = 0.0;
}

double QuantumMonteCarlo::getObservable(const std::string& name) const {
    auto it = observableValues_.find(name);
    if (it == observableValues_.end()) {
        throw std::out_of_range("Observable '" + name + "' not found");
    }
    return it->second;
}

double QuantumMonteCarlo::getObservableError(const std::string& name) const {
    auto it = observableErrors_.find(name);
    if (it == observableErrors_.end()) {
        throw std::out_of_range("Observable '" + name + "' not found");
    }
    return it->second;
}

void QuantumMonteCarlo::calculateErrorsAndDerivedQuantities() {
    double beta = 1.0 / params_.temperature;
    double N = static_cast<double>(model_->getLattice()->size());
    
    // Specific heat
    specificHeat_ = beta * beta * (energySquared_ - energy_ * energy_) / N;
    
    // Susceptibility
    susceptibility_ = beta * (magnetizationSquared_ - magnetization_ * magnetization_) / N;
    
    // Standard errors are calculated assuming binning has been done
